...
namespace ReShCaller
{
    ...
    public class ClientHandler
    {
        ...
        private readonly RSCallerClient _client;
        public ClientHandler(TcpClient tcpClient, RSCallerClient client)
        {
    ...
    public class RSCallerClient
    {
        public event Action<RSMessage> OnMessageReceived;
        public event Action<CommandResult> OnResultReceived;
        ...
        public RSCallerClient(string ipAddress, int port)
        {
            _serverIp = ipAddress;
            _serverPort = port;
        }
        public async Task<bool> ConnectAsync()
        {
            ...
            try
            {
                ...
                await SendMessage(RSMessageType.CallerLogin, new CallerLogin { UserName = "admin", Password = "password" });
        ...
        public void HandleNewMessage(RSMessage message)
        {
            ...
            {
                MyLogger.LogDebug($"[*]Error: null message");
            }
        }
        public async Task SendCommand(Guid clientId, CommandType type, object data)
        {
            await _clientHandler.SendCommand(new Command { ClientId = clientId, Type = type, Data = data });
        }
        public async Task SendMessage(RSMessageType type, object data)
        {
            await _clientHandler.SendMessageAsync(type, data);
        }
        private void ReceiveMessage(RSMessage message)
        {
            OnMessageReceived?.Invoke(message);
        }
        private void ReceiveCommandResult(CommandResult result)
        {
